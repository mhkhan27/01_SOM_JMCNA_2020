...
df$SD<-df$hh_size/((df$sum_rooms-(df_nona$kitchens+df_nona$toilets))*4)
...
######### Alberto / Issue ID-PEV-A1
library(dplyr)

df$A1 <- rep(NA, nrow(df))
df <- df %>% mutate(A1 = case_when(
                          household_expenditure == "adult_female" ~ 0,
                          household_expenditure == "adult_male" ~ 0,
                          TRUE ~ 1))
#########
...
######### Alberto / Issue ID-PEV-A2

df$A2 <- rep(NA, nrow(df))
df <- df %>% mutate(A2 = case_when(
                          breadwinner == "adult_female" ~ 0,
                          breadwinner == "adult_male" ~ 0,
                          TRUE ~ 1))
...
######### Alberto / Issue ID-PEV-B1

df$B1 <- rep(NA, nrow(df))
df <- df %>% mutate(B1 = case_when(
                          income_src.business == 1 ~ 0,
                          income_src.cash_fishing == 1 ~ 0,
                          income_src.livestock_production == 1 ~ 0,
                          income_src.contracted_job == 1 ~ 0,
                          income_src.cash_crop_farming == 1 ~ 0,
                          income_src.rent_of_land == 1 ~ 0,
                          TRUE ~ 1))
...
######### Alberto / Issue ID-PEV-B2

df$B2 <- rep(NA, nrow(df))
df <- df %>% mutate(B2 = case_when(
                         main_source_food.purchased_market == 1 ~ 0,
                         main_source_food.own_cultivation == 1 ~ 0,
                         main_source_food.own_livestock == 1 ~ 0,
                         TRUE ~ 1))
...
######### Alberto / Issue ID-PEV-B5

df$B5 <- rep(NA, nrow(df))
df <- df %>% mutate(B5 = case_when(
                          hh_members_new_unemployed != 0 & employ_loss_why.end_contract == 1 ~ 0,
                          hh_members_new_unemployed != 0 & employ_loss_why.other== 1 ~ 0, 
                          hh_members_new_unemployed != 0 & employ_loss_why.displacement == 1 ~ 1, 
                          hh_members_new_unemployed != 0 & employ_loss_why.locusts == 1 ~ 1, 
                          hh_members_new_unemployed != 0 & employ_loss_why.covid == 1 ~ 1, 
                          hh_members_new_unemployed != 0 & employ_loss_why.ill == 1 ~ 1, 
                          hh_members_new_unemployed != 0 & employ_loss_why.flooding == 1 ~ 1, 
                          hh_members_new_unemployed != 0 & employ_loss_why.drought == 1 ~ 1, 
                          hh_members_new_unemployed != 0 & employ_loss_why.conflict == 1 ~ 1)) 
...
######### Alberto / Issue ID-PEV-C1
df$C1<- rep(NA, nrow(df))
df <- df %>% mutate(C1 = case_when(
                          yes_no_relocate == "no" & days_displaced<366 ~ 0,
                          yes_no_relocate == "no" & days_displaced>=366 ~ 1))
...
######### Alberto / Issue ID-PEV-C2

df$C2<- rep(NA, nrow(df))
df <- df %>% mutate(C2 = case_when(
                        yes_no_relocate == "no" & displaced_locs<2 ~ 0,
                        yes_no_relocate == "no" & displaced_locs>1 ~ 1))
...
######### Alberto / Issue ID-PEV-F3

df$F3<- rep(NA, nrow(df))
df <- df %>% mutate(F3 = case_when(
                          health_time == "less15" ~ 0,
                          health_time =="16_30" ~ 0,
                          health_time =="31_60" ~ 0,
                          health_transport=="car" ~ 0,
                          health_transport=="cart" ~ 0,
                          health_transport=="moto" ~ 0,
                          health_transport=="bus" ~ 0,
                          TRUE ~ 1))
...
######### Alberto / Issue ID-PEV-F4
df$F4<- rep(NA, nrow(df))

df <- df %>% mutate(F4 = case_when(
                          market_time=="less15" ~ 0,
                          market_time=="min_15_29" ~ 0,
                          market_time=="min_30_59" ~ 0,
                          market_time != "dontknow" & market_transport=="car" ~ 0,
                          market_time != "dontknow" & market_transport=="cart" ~ 0,
                          market_time != "dontknow" & market_transport=="moto" ~ 0,
                          market_time != "dontknow" & market_transport=="bus" ~ 0,
                          market_transport =="walking" & market_time == "hr_1_2" ~ 1,
                          market_transport =="walking" & market_time == "more2h" ~ 1))
...
######### Alberto / Issue ID-PEV-F6
df$F6<- rep(NA, nrow(df))

df <- df %>% mutate(F6 = case_when(
                         hh_shelter != 0 & SD <= 2 ~ 0,
                         hh_shelter != 0 & SD > 2 ~ 1))
...
df$G4<- rep(NA, nrow(df))
df <- df %>% mutate(G4 = case_when(
                            HH_schoolaged_children > 0 & enrollement_note2<1 ~ 0,
                            HH_schoolaged_children > 0 & enrollement_note2 >= 1 ~ 1))
...
df$G5<- rep(NA, nrow(df))
df <- df %>% mutate(G5 = case_when(
                            school_time == "less15" ~ 0,
                            school_time == "16_30" ~ 0,
                            school_time == "31_60" ~ 0,
                            school_time != "dnk" & school_transport=="car" ~ 0,
                            school_time != "dnk" & school_transport=="cart" ~ 0,
                            school_time != "dnk" & school_transport=="moto" ~ 0,
                            school_time != "dnk" & school_transport=="bus" ~ 0,
                            school_time == "above180" & school_transport=="walking" ~ 1,
                            school_time == "60_180" & school_transport=="walking" ~ 1))
...
df$G9<- rep(NA, nrow(df))
df$G9[df$HH_schoolaged_children>0 & (df$school_barrier_note.no_barriers==1| df$school_barrier_note.other==1)]<-1
df$G9[df$HH_schoolaged_children>0 & (df$school_barrier_note.displaced==1| df$school_barrier_note.poor_performance==1 | df$school_barrier_note.curriculum==1| df$school_barrier_note.parents==1 |df$school_barrier_note.no_interest==1)]<-2
df$G9[df$HH_schoolaged_children>0 & (df$school_barrier_note.schools_closed==1| df$school_barrier_note.schools_overcrowded==1 | df$school_barrier_note.distance_to==1| df$school_barrier_note.school_fees==1 |df$school_barrier_note.registration==1 | df$school_barrier_note.overcrowded==1| df$school_barrier_note.lack_teachers==1 | df$school_barrier_note.poor_infrastrcture==1| df$school_barrier_note.wash==1 |df$school_barrier_note.wash_gender==1)]<-3
df$G9[df$HH_schoolaged_children>0 & (df$school_barrier_note.no_school==1| df$school_barrier_note.discrimination_enroll==1 | df$school_barrier_note.cannot_go==1| df$school_barrier_note.chores==1)]<-3
df$G9[df$HH_schoolaged_children>0 & df$school_barrier_note.security_concerns==1]<-3

df$G10<- rep(NA, nrow(df))
df$G10[df$HH_schoolaged_children>0 & (df$school_barrier_girls_note.no_barriers==1| df$school_barrier_girls_note.other==1)]<-1
df$G10[df$HH_schoolaged_children>0 & (df$school_barrier_girls_note.displaced==1| df$school_barrier_girls_note.poor_performance==1 | df$school_barrier_girls_note.curriculum==1| df$school_barrier_girls_note.parents==1 |df$school_barrier_girls_note.no_interest==1)]<-2
df$G10[df$HH_schoolaged_children>0 & (df$school_barrier_girls_note.schools_closed==1| df$school_barrier_girls_note.schools_overcrowded==1 | df$school_barrier_girls_note.distance_to==1| df$school_barrier_girls_note.school_fees==1 |df$school_barrier_girls_note.registration==1 | df$school_barrier_girls_note.overcrowded==1| df$school_barrier_girls_note.lack_teachers==1 | df$school_barrier_girls_note.poor_infrastrcture==1| df$school_barrier_girls_note.wash==1 |df$school_barrier_girls_note.wash_gender==1)]<-3
df$G10[df$HH_schoolaged_children>0 & (df$school_barrier_girls_note.no_school==1| df$school_barrier_girls_note.discrimination_enroll==1 | df$school_barrier_girls_note.cannot_go==1| df$school_barrier_girls_note.chores==1)]<-3
df$G10[df$HH_schoolaged_children>0 & df$school_barrier_girls_note.security_concerns==1]<-3
...
#### Alberto / calcualted without G1

df_G_crit<-data.frame(df$G9, df$G10)
...
df$H1[(df$health_household.yes_diarhea==1 | df$health_household.yes_fever==1 |df$health_household.yes_cough==1 | df$health_household.yes_skin==1 |df$health_household.yes_eye==1 | df$health_household.yes_wound==1) & (df$health_transport=="walking"|df$health_transport=="have_not"|df$health_transport=="treatment_toofar") & (df$health_time=="60_180"|df$health_time=="above180") ]<-4
...
df$H2<- rep(NA, nrow(df))
df <- df %>% mutate(H2 = case_when(
                          health_access == "yes" ~ 0,
                          health_access == "no_seek" ~ 0,
                          health_access == "no" ~ 1))

## The skip logics seems to be there already...
df_check <- df %>% select(health_household.none, health_household.dontknow, health_access, H2)
...
df$H3<- rep(NA, nrow(df))
df <- df %>% mutate(H3 = case_when(
                          health_access == "no" ~ 1,
                          
                          health_access != "no_seek" & health_seek.relative_friend==1 ~ 1,
                          health_access != "no_seek" & health_seek.shop_street==1 ~ 1,
                          health_access != "no_seek" & health_seek.other==1 ~ 1,
                          health_access != "no_seek" & health_seek.traditional_practitioner==1 ~ 1,
            
                          
                          health_access != "no_seek" & health_seek.private_clinic==1 ~ 0,
                          health_access != "no_seek" & health_seek.private_physician==1 ~ 0,
                          health_access != "no_seek" & health_seek.private_pharmacy==1 ~ 0,
                          health_access != "no_seek" & health_seek.other_private_medical==1 ~ 0,
                          health_access != "no_seek" & health_seek.gov_hospital==1 ~ 0,
                          health_access != "no_seek" & health_seek.government_center==1 ~ 0,
                          health_access != "no_seek" & health_seek.government_post==1 ~ 0,
                          health_access != "no_seek" & health_seek.other_public==1 ~ 0,
                          health_access != "no_seek" & health_seek.community_worker==1 ~ 0,
                          health_access != "no_seek" & health_seek.mobile_clinic_outreach==1 ~ 0,
                          health_access != "no_seek" &health_seek.community_health_worker==1~ 0,
                          health_access != "no_seek" & health_seek.mobile_clinic==1 ~ 0))
...
######### Alberto / ID-HLT-H5
df$H5<- rep(NA, nrow(df))
df <- df %>% mutate(H5 = case_when(
                              unvaccinated_why.cost_high== 1 ~ 1,
                              unvaccinated_why.problems_civil== 1 ~ 1,
                              unvaccinated_why.public_clinic== 1 ~ 1,
                              unvaccinated_why.treatment_toofar== 1 ~ 1,
                              unvaccinated_why.no_medicine== 1 ~ 1,
                              unvaccinated_why.no_treament== 1 ~ 1,
                              unvaccinated_why.medical_refused== 1 ~ 1,
                              unvaccinated_why.no_pwd== 1 ~ 1,
  
                              unvaccinated_child == "no" ~ 0,
                              unvaccinated_why.no_issues==1 ~ 0,
                              unvaccinated_why.have_not==1 ~ 0,
                              unvaccinated_why.public_not_open==1 ~0,
                              unvaccinated_why.lack_staff== 1 ~ 0))
                              
# df_check <- df %>% select(unvaccinated_child, starts_with("unvaccinated_why."), H5) %>% filter(unvaccinated_child == "yes")                       
                              

#########

df$H6<- rep(NA, nrow(df))
df$H6[df$birth_where=="doctor" | df$birth_where=="nurse" | df$birth_where=="health_center" | df$birth_where=="government_health" |  df$birth_where=="government_clinic"| df$birth_where=="government_hospital" | df$birth_where=="other_public" | 
        df$birth_where=="private_hospital" | df$birth_where=="private_clinic" | df$birth_where=="private_maternity" | df$birth_where=="other_private"]<-0
df$H6[df$birth_where=="respondent_s" | df$birth_where=="other_home" ]<-1

######### Alberto / ID-HLT-H6
df$H6<- rep(NA, nrow(df))
df <- df %>% mutate(H6 = case_when(
                            pregnancy == "yes" & hh_female > 0 & birth_where=="respondent_s" ~ 1,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="other_home" ~ 1,
                            
                            pregnancy == "yes" & hh_female > 0 & birth_where=="doctor" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="nurse" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="health_center" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="government_health" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="government_clinic" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="government_hospital" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="other_public" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="private_hospital" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="private_clinic" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="private_maternity" ~ 0,
                            pregnancy == "yes" & hh_female > 0 & birth_where=="other_private" ~ 0))

# df_check <- df %>% select(pregnancy, hh_female,  birth_where, H6)                   

#########

df$H7<- rep(NA, nrow(df))
df$H7[df$who_assist=="doctor" | df$who_assist=="nurse" | df$who_assist=="other_health" | df$who_assist=="traditional" | df$who_assist=="community"]<-1
df$H7[df$who_assist=="relative" | df$who_assist=="other" ]<-3
df$H7[df$who_assist=="no" ]<-"4+"

######### Alberto / ID-HLT-H7 --- I think the skip logic is already included

#df_check <- df %>% select(birth_where, who_assist, H7) %>% filter(birth_where == "no_birth")
...
######### Alberto / ID-HLT-H10
df$H10<- rep(NA, nrow(df))
df <- df %>% mutate(H10 = case_when(
                            barriers_health.cost_high==1 ~ 1,
                            barriers_health.problems_civil==1 ~ 1,
                            barriers_health.public_clinic==1 ~ 1,
                            barriers_health.treatment_toofar==1 ~ 1,
                            barriers_health.no_medicine==1 ~ 1,
                            barriers_health.no_treament==1 ~ 1,
                            barriers_health.medical_refused==1 ~ 1,
                            barriers_health.no_pwd==1 ~ 1,
                            
                            barriers_health.no_issues==1 ~ 0,
                            barriers_health.have_not==1 ~ 0,
                            barriers_health.lack_staff==1 ~ 0,
                            barriers_health.public_not_open==1 ~ 0))

# df_check <- df %>% select(starts_with("barriers_health."), H10) %>% filter(barriers_health.lack_staff==1)
...
df$I1[df$unusually_sleepy=="yes_7more" | df$child_fever=="yes_7more"]<-4  #### ID-NUT-I1
...
##### Alberto I3 - skip logic updated

df$I3<- rep(NA, nrow(df))
df <- df %>% mutate(I3 = ifelse(hh_children >0 & child_thin == "yes" & eating_normally == "yes_eating", 1,
                                ifelse(hh_children >0 & child_thin == "yes" & eating_normally == "no_less2", 3, 
                                       ifelse(hh_children >0 & child_thin == "yes" & eating_normally == "no_less3", 4,
                                              ifelse(hh_children >0 & child_thin == "no", 1, NA)))))



df_check <- df %>% select(hh_children, child_thin, eating_normally, I3) %>% filter(hh_children == 0)
...
df$I4[df$nutrition_enroll=="no"]<-0
df$I4[df$nutrition_enroll=="yes"]<-1
...
######### Alberto / ID-NUT-I5 -- skip logic already there...

# df_check <- df %>% select( nearest_nutrition, nutrition_time, Nutrition_transport, I5) %>% filter( nearest_nutrition == "no" |  nearest_nutrition == "not_needed")
...
######### Alberto / ID-NUT-I6 --- nothing on the dap /// nothing to do
...
df$J3<- rep(NA, nrow(df))
df$J3[df$rCSI>=0 & df$rCSI<=18]<-1
df$J3[df$rCSI>=19 & df$rCSI<=56]<-0
...
df_J_crit<-data.frame(df$J1, df$J2)
...
df$K3[df$enough_water.other__domestic_purposes_==0] <-0
df$K3[df$enough_water.personal_hygiene_==0] <-1
df$K3[df$enough_water.cooking==0] <-1
df$K3[which(df$enough_water.not_enough_water_==1 & (df$enough_water.drinking==1 | df$enough_water.cooking==1 | df$enough_water.personal_hygiene_==1 | df$enough_water.other__domestic_purposes_==1))]<-NA
...
df$K6[(df$sanitation_facility == "none_of" | df$sanitation_facility == "open_hole" | df$sanitation_facility == "dontknow")]<-1
...
# df_check <- df %>% select(sanitation_facility, starts_with("latrine_feature"), K6)
...
#### Alberto K9 to binary

df$K9<- rep(NA, nrow(df))
df$K9[df$of_water.doesn_t_have_issue==1]<-0
df$K9[df$of_water.less_preferred_cooking==1 | df$of_water.surface_water_cooking==1| df$of_water.unsual_source==1]<-0
df$K9[df$of_water.rely_less_preferred==1 | df$of_water.dangerious_other==1]<-1
df$K9[df$of_water.or_credit_for==1 | df$of_water.consumption_less==1]<-1
df$K9[df$of_water.surface_water==1| df$of_water.send_children_water==1 | df$of_water.dangerious_source==1 ]<-1
...
df$K10<- rep(NA, nrow(df))
df <- df %>% mutate(K10 = case_when(
                            sanitation_barriers.sanitation_full==1 ~ 1,
                            sanitation_barriers.sanitation_far==1 ~ 1,
                            sanitation_barriers.sanitation_disabilities==1 ~ 1,
                            sanitation_barriers.some_toilets==1 ~ 1,
                            sanitation_barriers.sanitation_women==1 ~ 1,
                            sanitation_barriers.sanitation_etc==1 ~ 1,
                            sanitation_barriers.going_dangerous==1 ~ 1,
  
                            sanitation_barriers.no_problem==1 ~ 0,
                            sanitation_barriers.lack_crowded==1 ~ 0,
                            sanitation_barriers.sanitation_unhygienic==1 ~ 0))

#########

df$K11<- rep(NA, nrow(df))
df$K11[df$sanitation_coping.no_problem==1]<-1
df$K11[df$sanitation_coping.latrines_toilets==1]<-2
df$K11[df$sanitation_coping.less_prefered_toilets==1]<-3
...
##### Alberto K11 binary
df$K11<- rep(NA, nrow(df))
df$K11[df$sanitation_coping.no_problem==1]<-0
df$K11[df$sanitation_coping.latrines_toilets==1]<-0
df$K11[df$sanitation_coping.less_prefered_toilets==1]<-1
df$K11[df$sanitation_coping.usual_one==1 | df$sanitation_coping.dangerous_place==1 | df$sanitation_coping.at_night==1]<-1
df$K11[df$sanitation_coping.plastic_bag==1 | df$sanitation_coping.the_open==1]<-1
...
##### Alberto - K12 to binary


df$K12<- rep(NA, nrow(df))
df$K12[df$hygeine_coping.any_issue==1]<-0
df$K12[df$hygeine_coping.rely_less_preferred==1 | df$hygeine_coping.buying_at_further==1]<-0
df$K12[df$hygeine_coping.rely_soap_substitutes==1 | df$hygeine_coping.borrow_from_relative==1 | df$hygeine_coping.spend_or_used==1 | df$hygeine_coping.reduce_consumption_consumption==1 | df$hygeine_coping.reduce_consumption_cleaning==1]<-1
df$K12[df$hygeine_coping.buying_at_dangerious==1 ]<-1
...
df_K_bin<-data.frame(df$K3, df$K6, df$K10, df$K7, df$K13)
...
######### Alberto / ID-SNFI-L1
df$L1<- rep(NA, nrow(df))
df$L1[df$hh_shelter>0 & df$SD<=2]<-0
df$L1[df$hh_shelter>0 & df$SD>2]<-1

# df_check <- df %>% select(hh_shelter, SD, L1)
...
######### Alberto / ID-SNFI-L3
df$L3<- rep(NA, nrow(df))
df$L3[df$hh_shelter>0 & (df$lack_enclosure.none_of==1 | df$lack_enclosure.leaks_light_rain==1 | df$lack_enclosure.limited_room==1 | df$lack_enclosure.presence_removable==1)]<-0
df$L3[df$hh_shelter>0 & (df$lack_enclosure.lack_insulation==1 | df$lack_enclosure.leaks_heavy_rain==1 | df$lack_enclosure.presence_debris==1)]<-1

# df_check <- df %>% select(hh_shelter, starts_with("lack_enclosure."), L3) %>% filter(hh_shelter == 0)
...
######### Alberto / ID-SNFI-L3
df$L4<- rep(NA, nrow(df))
df$L4[df$hh_shelter>0 & (df$shelter_damage.none_of==1 |df$shelter_damage.opening_cracks==1 | df$shelter_damage.broken_or==1 | df$shelter_damage.some_in==1 | df$shelter_damage.damaged_Damage==1  | df$shelter_damage.foundation_damaged==1| df$shelter_damage.gas_sewage==1 | df$shelter_damage.electricity_and==1  | df$shelter_damage.other==1)]<-1
df$L4[df$hh_shelter>0 & (df$shelter_damage.roof_Roof==1 | df$shelter_damage.exterior_to==1 | df$shelter_damage.exterior_or==1 | df$shelter_damage.large_in==1 | df$shelter_damage.total_Total==1 | df$shelter_damage.some_walls==1)]<-3
df$L4[df$hh_shelter>0 & df$shelter_damage.severe_unsafe==1]<-4

# df_check <- df %>% select(hh_shelter, starts_with("shelter_damage."), L4) %>% filter(hh_shelter == 0)
...
######### Alberto / ID-SNFI-L6
df$L6<- rep(NA, nrow(df))
df$L6[df$hh_shelter>0 & (df$shelter_issues.none==1 |df$shelter_issues.unable_home==1 | df$shelter_issues.other==1) ]<-0
df$L6[df$hh_shelter>0 & (df$shelter_issues.lack_partitions==1 |df$shelter_issues.lack_inside==1 | df$shelter_issues.cooking_are==1 | df$shelter_issues.lack_per==1 | df$shelter_issues.lack_around==1  | df$shelter_issues.bathing_are==1| df$shelter_issues.lack_bathing==1 | df$shelter_issues.lack_cooking==1  | df$shelter_issues.theft==1| df$shelter_issues.other_security==1 | df$shelter_issues.fire==1  | df$shelter_issues.poor_of==1)]<-1

# df_check <- df %>% select(hh_shelter, starts_with("shelter_damage."), L6) %>% filter(hh_shelter == 0)
...
######### Alberto / ID-SNFI-L8
df$L8<- rep(NA, nrow(df))
df$L8[df$hh_shelter>0 & (df$occupancy_arrangement=="ownership" | df$occupancy_arrangement=="rented" |df$occupancy_arrangement=="hosted")]<-0
df$L8[df$hh_shelter>0 & (df$occupancy_arrangement=="no_occu" | df$occupancy_arrangement=="other") ]<-1

# df_check <- df %>% select(hh_shelter, occupancy_arrangement, L8) %>% filter(hh_shelter == 0)
...
######### Alberto / ID-SNFI-L9
df$L9<- rep(NA, nrow(df))
df$L9[df$hh_shelter>0 & df$written_documentation=="yes" ]<-0
df$L9[df$hh_shelter>0 & df$written_documentation=="no"]<-1
...
df$M1<- rep(NA, nrow(df))
df$M1[df$child_labor_note=="no" & df$HH_schoolaged_children>0]<-0
df$M1[df$child_labor_note=="yes" &df$HH_schoolaged_children>0]<-1
...
######### Alberto / ID-PROT-M2
df$M2<- rep(NA, nrow(df))
df$M2[df$children_away=="no" | df$child_study>0]<-0
df$M2[df$child_employment>0 | df$child_married>0 | df$child_armed>0 | df$child_abducted>0 | df$child_missing>0 | df$child_detained>0]<-1
...
######### Alberto / ID-PROT-M4
df$M4<- rep(NA, nrow(df))
df$M4[df$not_safe_boys=="no"]<-0
df$M4[df$not_safe_boys=="yes"]<-1
...
######### Alberto / ID-PROT-M6
df$M6<- rep(NA, nrow(df))
df$M6[df$not_safe_girls=="no"]<-0
df$M6[df$not_safe_girls=="yes"]<-1
...
######### Alberto / ID-PROT-M11
df$M11<- rep(NA, nrow(df))
df$M11[df$gbv_incidents=="no"]<-0
df$M11[df$gbv_incidents=="yes"]<-1
...
df_M_crit<-data.frame(df$M8, df$M3)
...
######### Alberto / ID-AAP-N7
df$N7<- rep(NA, nrow(df))
df$N7[df$aid_access == "yes" & (df$aid_barriers.lack_information==1 |df$aid_barriers.physical_access==1 | df$aid_barriers.other==1)]<-0
df$N7[df$aid_access == "yes" & (df$aid_barriers.insecure_route==1 |df$aid_barriers.insecure_site==1 | df$aid_barriers.exclusion_gatekeepers==1)]<-1

# df_check <- df %>% select(aid_access, starts_with("aid_barriers."), N7) %>% filter(aid_access != "yes")
...
#N19: not scored: length(dot_multiples("services_affected[.]",df))
